# preprocessing script to bin and impute final data
...
def bin_and_impute(data, bin_width=60, variable_start_index=5):
    ...
    # create string for resample parameter
    ...
    for icustay_id in id_s:
        ...
        # select subset of dataframe featuring current icustay_id
        pat = data.query("icustay_id == @icustay_id")
        pat_i = pat.set_index('chart_time', inplace=False)

        # resampling needs either datetime or timedelta format
        # create dummy timestamp from relative hour:
        
        # pat_i.index = pd.to_datetime(pat_i.index, unit='D') 
        # could try #unit='s'
        ...
        if pat_i.index[0] > start:
            ...
            first_row = pd.DataFrame(
                first_row, columns=pat_i.columns, index=[start])
        ...
        # loffset=pat_i.index[0], label='left'
        pat_rs = pat_a[variables].resample(bin_width, how='mean')
        ...
        for variable in variables:
            ...
            if n_nans > 0:  # if yes, this variable/channel has still NaNs --> address with mean imputation
                ...
                pat_ff[variable] = pat_ff[variable].replace(
                    np.nan, replacement)
